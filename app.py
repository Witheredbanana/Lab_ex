...
def init_db():
    ...
    with db.engine.connect() as conn:
        try:
            ...
            print(f"Ошибка при удалении поля status_id: {e}")
            conn.rollback()
    ...
    if User.query.filter_by(login='admin').first() is None:
        ...
        # Добавляем рецензии на "Мастер и Маргарита"
        master_book = Book.query.filter_by(title='Мастер и Маргарита').first()
        if master_book:
            reviews = [
                Review(
                    book_id=master_book.id,
                    user_id=test_user.id,
                    rating=4,
                    text='Очень интересная книга! Особенно понравились главы с Воландом и его свитой. Автор мастерски переплетает реальность и фантастику, создавая уникальную атмосферу. Единственное, что немного смутило - это некоторые длинные философские отступления.',
                ...
                Review(
                    ...
                    user_id=test_user.id,
                    rating=2,
                    text='К сожалению, книга не оправдала моих ожиданий. Сюжет показался запутанным, а некоторые персонажи - непонятными. Возможно, нужно перечитать её позже, чтобы лучше понять замысел автора.',
    ...
    if Book.query.count() == 0:
        ...
        books = [
            Book(
                ...
                year=1967,
                pages=480,
                description='Роман «Мастер и Маргарита» — самое известное произведение Михаила Булгакова. В нем переплетаются реальность и фантастика, история и современность, любовь и ненависть, добро и зло.',
                genres=[novel],
                cover=covers['master']
            ),
            Book(
                ...
                year=1866,
                pages=672,
                description='Роман «Преступление и наказание» — одно из самых известных произведений Федора Достоевского. В нем исследуются темы морали, совести и искупления.',
            ...
            Book(
                ...
                year=1833,
                pages=320,
                description='Роман в стихах «Евгений Онегин» — самое известное произведение Александра Пушкина. В нем отражена жизнь русского общества первой трети XIX века.',
                genres=[poetry, novel],
                cover=covers['onegin']
            ),
            Book(
                ...
                year=1869,
                pages=1225,
                description='Роман-эпопея «Война и мир» — одно из самых значительных произведений мировой литературы. В нем отражена жизнь русского общества в эпоху наполеоновских войн.',
                genres=[novel],
                cover=covers['war']
            ),
            Book(
                ...
                year=1997,
                pages=432,
                description='Первый роман в серии книг о юном волшебнике Гарри Поттере. История о мальчике, который узнает, что он волшебник, и отправляется в школу магии Хогвартс.',
                genres=[fantasy],
                cover=covers['potter']
            )
        ]
...
def statistics():
    if current_user.role.name != 'Администратор':
        flash('У вас нет прав для просмотра статистики')
...
def edit_book(book_id):
    if current_user.role.name != 'Администратор':
        flash('У вас нет прав для редактирования книг')
        return redirect(url_for('index'))
...
def delete_book(book_id):
    if current_user.role.name != 'Администратор':
        flash('У вас нет прав для удаления книг')
        return redirect(url_for('index'))
